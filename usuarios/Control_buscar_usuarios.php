...
if (isset($_POST["buscadorcategoria"])&&isset($_POST["buscadornombre"])) {
	...
	switch ($php_selected ) {
		case 1:
			//Si buscan por id
			$query.="WHERE idUsuario LIKE ?;";
			break;
		case 2:
			//Si buscan por nombre LIKE
			$query.="WHERE nombreUsuario LIKE ?;";
			break;
		case 3:
			//Si buscan por USUARIO LIKE
			$query.="WHERE nickUsuario LIKE ?;";
			break;

		case 4:
			//Si buscan por USUARIO LIKE
			$query.="WHERE emailUsuario LIKE ?;";
			break;
	...
	while($fila = $result->fetch_assoc()){
		...
		<td><?php echo htmlspecialchars($fila['nickUsuario']); ?></td>
		<td><?php echo htmlspecialchars($fila['emailUsuario']); ?></td>


		<!--$connection->monkeyCrypt(htmlspecialchars($fila['idUsuario']),1);-->
		<?php $phpid = "id=".$connection->monkeyCrypt(htmlspecialchars($fila['idUsuario'].'(*)'.$fila['nombreUsuario']),1);?>
		
		<td><a href='javascript:eliminar(<?php echo json_encode($phpid); ?>)' class="button fit small">Borrar</a></td>

		<td><a  class="button fit small" href="<?php echo 'editar.php?id='.$connection->monkeyCrypt(htmlspecialchars($fila['idUsuario']),1); ?>">Editar</a></td>
